...
<div class="loop-meta">
	...
	<?php } elseif ( is_search() ) { ?>
		...
		<div class="loop-description">
			<?php echo wpautop( sprintf( __( 'You are browsing the search results for "%s"', 'fox-roots' ), esc_attr( get_search_query() ) ) ); ?>
		</div><!-- .loop-description -->
	...
	<?php } elseif ( is_post_type_archive() ) { ?>
		...
		<?php if ( is_post_type_archive( 'portfolio_item' ) ) get_template_part( 'menu', 'portfolio' ); // Get portfolio menu. ?>

	<?php } elseif ( is_day() || is_month() || is_year() ) { ?>

		<?php
			if ( is_day() )
				$date = get_the_time( __( 'F d, Y', 'fox-roots' ) );
			elseif ( is_month() )
				$date = get_the_time( __( 'F Y', 'fox-roots' ) );
			elseif ( is_year() )
				$date = get_the_time( __( 'Y', 'fox-roots' ) );
		?>

		<h1 class="loop-title"><?php echo $date; ?></h1>

		<div class="loop-description">
			<?php echo wpautop( sprintf( __( 'You are browsing the site archives for %s.', 'fox-roots' ), $date ) ); ?>
		</div><!-- .loop-description -->

	<?php } elseif ( is_archive() ) { ?>

		<h1 class="loop-title"><?php _e( 'Archives', 'fox-roots' ); ?></h1>

		<div class="loop-description">
			<?php echo wpautop( __( 'You are browsing the site archives.', 'fox-roots' ) ); ?>
		</div><!-- .loop-description -->
